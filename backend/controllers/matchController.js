...
// Orchestrates the matching pipeline: for a given Job, fetch candidate
// resumes, call the ML microservice for semantic similarity + skill
// overlap, combine into a weighted final score, persist MatchScore docs,
// and return candidates ranked best-to-worst.
...
// Weights for the final composite score (tune per business needs)
...
const runMatchingForJob = async (req, res, next) => {
  try {
    ...
    const resumes = await Resume.find({ status: "parsed" }).populate("candidate");
    if (resumes.length === 0) {
      return res.status(200).json({ success: true, message: "No parsed resumes to match", data: [] });
    }

    // Batch call to ML service: send JD + all resume texts/skills, get back
    // similarity scores + skill overlap in one round trip for efficiency.
    const payload = {
    ...
    const { data } = await axios.post(`${ML_SERVICE_URL}/match-candidates`, payload, { timeout: 60000 });
    // Expected response: { results: [{ resume_id, semantic_similarity, skill_match_score,
    //   experience_match_score, education_match_score, matched_skills, missing_skills, insight }] }
    ...
    const scored = data.results.map((r) => {
      const finalPct =
        ...
        100;
      return { ...r, finalMatchPercentage: Math.round(finalPct * 100) / 100 };
    });

    // Rank descending by final match percentage
    scored.sort((a, b) => b.finalMatchPercentage - a.finalMatchPercentage);
    ...
    res.status(200).json({ success: true, message: "Matching complete", data: bulkResults });
  } catch (error) {
    next(error);
  }
};
...
// @desc   Get all job matches for a specific candidate (candidate's own dashboard)
// @access Private (candidate)
const getMatchesForCandidate = async (req, res, next) => {
  try {
    const matches = await MatchScore.find({ candidate: req.params.candidateId })
      .sort({ finalMatchPercentage: -1 })
...
module.exports = { runMatchingForJob, getMatchesForJob, getMatchesForCandidate };
